...
module Fastest
  ...
  module Linux
    ...
    class Platform < Fastest::Unix::Platform
      ...
      def default_browser
        File.expand_path(`update-alternatives --list x-www-browser`.lines.first.chomp)
      end

      # @return [String] path to default (graphical) text editor
      def default_text_editor
        File.expand_path(`update-alternatives --list gnome-text-editor`.lines.first.chomp)
      end
